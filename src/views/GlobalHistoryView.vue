...
async function fetchMostFreqPair() {
  isLoading.value = true

  try {
    const response = await fetch('/api/trade/most-freq-pair', {
      method: 'GET',
    ...
    if (response.ok) {
      if (data.pairs && Array.isArray(data.pairs)) {
        records.value = data.pairs.map((item, index) => ({
          id: index + 1,
          leftName: item.pair[0],
          leftCount: '',
          rightName: item.pair[1],
          rightCount: `${item.count}x`,
          timestamp: null
        }))
      } else {
        records.value = []
      ...
      console.error('Failed to fetch most frequent trading pairs:', data)
      records.value = []
    }
  } catch (error) {
